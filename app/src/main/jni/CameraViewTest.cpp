...
void yv12gl_display(const unsigned char * y, const unsigned char *u,const unsigned char *v, int width, int height, unsigned long long time)
{
	...
	if(g_yuv_display == NULL){LOGE("g_yuv_display==NULL"); return;}
	if (!g_yuv_display->enable){LOGE("!enable");return;}
...
#if 0
static void convert_head(T_STM_STRUCT* pstream,stream_head* hw_head)
{
	hw_head->tag = HW_MEDIA_TAG;
	hw_head->sys_time = time(NULL);
	hw_head->len = sizeof(stream_head) + pstream->size;
	hw_head->type = pstream->iFrame ? 1: 0;
//	hw_head->time_stamp = (unsigned long long)pstream->timestamp * 1000;
	struct timeval val;
	gettimeofday(&val,NULL);
	uint64_t timestamp = (unsigned long long)val.tv_sec * 1000 + val.tv_usec / 1000;
	hw_head->time_stamp = timestamp *1000;
...
void process_stream(){
	stream_head sh;
	convert_head(pstream,&sh);

	int video_type = 0;
	if(is_sub == 0)
	{
		video_type = pstream->iFrame ? HW_FRAME_VIDEO_I: HW_FRAME_VIDEO_P;
	}
	else
	{
		video_type = pstream->iFrame ? HW_FRAME_VIDEO_SUB_I: HW_FRAME_VIDEO_SUB_P;
	}
}
#endif
