...
import camera.utils.CamParaUtil;
import camera.utils.ImageUtil;
...
public class CameraInterface {
	...
	private int maxZoom;
	private int mCurZoomVal;
	private ICameraInterface mInterfaceCallback;
//	private byte [] mbuffer = new byte [2*1024*1024];
	...
	public void setCameraInterfaceCallback(ICameraInterface cb){
		mInterfaceCallback = cb;
	}
		...
		if(mCamera != null){  
			mCamera.setZoomChangeListener(new Camera.OnZoomChangeListener() {
				...
				public void onZoomChange(int zoomValue, boolean stopped, Camera camera) {
					Log.i("123","zoomValue="+zoomValue+"  stop="+stopped);
					mCurZoomVal = zoomValue;
				}
			});
			...
			mParams.setPreviewFpsRange(30000, 60000);
...
//			mParams.setPreviewSize(600,300);
...
//				FileUtil.saveBitmap(rotaBitmap);
			...
			if (mInterfaceCallback!=null){
				mInterfaceCallback.takePhotoFinish(rotaBitmap);
			}
			b.recycle();
			b = null;
		}  
	...
	public void zoomIn(){
		...
		mParams = mCamera.getParameters();
//		mCurZoomVal = mParams.getZoom();
		Log.i("123","is zoom support="+mParams.isZoomSupported()+"   smoothzoom="+mParams.isSmoothZoomSupported()+"   cur="+mCurZoomVal);
		if (mParams.isZoomSupported()) {

			if (mCurZoomVal<maxZoom){
				mCamera.startSmoothZoom(mCurZoomVal++);
//				mParams.setZoom(mCurZoomVal++);
//				mCamera.setParameters(mParams);
//				mCamera.startPreview();
			}
		}
